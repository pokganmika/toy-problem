...
const toFraction = (number: any): string => {
  // union type?
  // Your code here

  const frac = (number: any): string => {
    if (number % 1 === 0) {
      // x. 0
      return `${number.toString()}/1`;
    } else if (number % 1 !== 0) {
      const checkFirst: string = number.toString()[0];

      if (checkFirst === '0') {
        // 0. ...
        let child: string | number = '';
        let parent: string | number = '1';
        let char: string = number.toString();
